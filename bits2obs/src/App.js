...
function App() {
  ...
  const [isStreamlabsConnected, setIsStreamlabsConnected] = useState(localStorage.getItem("streamlabs_socket_token") !== null);
  ...
  const disconnectStreamlabs = () => {
    ...
    localStorage.removeItem("streamlabs_socket_token");
      ...
      fetch("https://bits2obs.zappatic.net/streamlabs_token.php?code=" + qs.get("code"))
        ...
        .then((data) => {
          if (data.hasOwnProperty("error")) {
            ...
            fetch("https://streamlabs.com/api/v1.0/socket/token?access_token=" + data.access_token)
              .then(function (res) {
                return res.json();
              })
              .then(function (json) {
